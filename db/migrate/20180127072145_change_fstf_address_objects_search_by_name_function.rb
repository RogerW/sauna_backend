class ChangeFstfAddressObjectsSearchByNameFunction < ActiveRecord::Migration[5.1]
  def up
    connection.execute(%q{
      DROP FUNCTION IF EXISTS fstf_AddressObjects_SearchByName(
        ...
        RETURNS  TABLE (rtf_AddressObjectsFullName TEXT)
        AS
          $BODY$
        DECLARE
        ...
        BEGIN
          ...
          IF a_ParentFormalName IS NULL AND a_ParentShortName IS NULL 
            ...
            RETURN QUERY
                        SELECT concat_ws(';', cfa.AOGUID,cfa.AOLevel,
          ...
          ELSIF a_ParentFormalName IS NOT NULL
            ...
            v_FormalNameTemplate:=COALESCE(v_FormalNameTemplate,c_WildChar);
                        RETURN QUERY 
                        SELECT concat_ws(';', cfa.AOGUID,cfa.AOLevel,fsfn_AddressObjects_TreeActualName(cfa.AOGUID),
