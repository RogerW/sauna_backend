class ReservationsController < ApplicationController
  ...
  def create
    ...
    if @model.save
      ...
      render json: {errors: @model.errors, msg: @model.errors.full_messages.join(', ')}, status: 422
    end
  end
  ...
  def set_model
    if params[:sauna_list_id].present?
      @model = Sauna.find(params[:sauna_list_id]).reservations
    else
      @model = Reservation
    end
  end
